...
options = {
	...
	insertOptionsLink : function(){
		$('div.floatl.vcenter.elf.dgray:last').after('<div class="floatl vcenter" style="padding-top:4px;"><img src="http://images.readmore.de/img/header/line.jpg" alt="" style="height:25px; width:2px;"></div><div class="floatl vcenter elf dgray" style="margin:11px 10px;"><a onclick="$(\'#userscriptOptions\').toggle();" id="openUserscriptOptions" href="#" class="black">Userscript Optionen</a></div>');
		return false;
	},
...
leftColumn = {
	...
	streams : {
		...
		hideSelectedStreams : function(){
			...
			$('#leftc>div.block:eq(1)>div>div>img, #leftc>div.block:eq(2)>div>div>img').each(function(index, element){
	...
	*************************/
	forum : {
		...
		readThreadlink : function (){
			middleColumn.forum.threadlink = $(location).attr('href').replace(/\&pagenum=.+$/, '');
			return false;
		},
		...
		*************************/
		reloadPosts : {
			...
			readPostcount : function (){
				middleColumn.forum.reloadPosts.postcount = $('[class^=post_]').length;
				return false;
			},
			...
			readNewPosts : function(){
				...
				$.ajax({
					...
					async: true,
					cache: false,
					url: middleColumn.forum.threadlink + '&pagenum=' + middleColumn.forum.page,
			...
			showNewPostsTitle : function(){
				if(middleColumn.forum.reloadPosts.oldTitle == '') middleColumn.forum.reloadPosts.oldTitle = $('title').text();
				var title = middleColumn.forum.reloadPosts.oldTitle;
				title = '(' + middleColumn.forum.reloadPosts.unseenPosts.length + ') ' + title;
				$('title').text(title);	
			},
		...
		preview : {			
			...
			initializePreview : function(){
				...
				var thirdRow = '<span style="font-size: 11px;"><img style="height: 11px;" src="http://images.readmore.de/img/icons/online.gif"><img src="http://thextor.de/readmore-userscript/img/space.gif" style="border: none; height: 1px; width: 5px;"><a class="bml" href="index.php?cont=profile&amp;id=' + middleColumn.forum.preview.userid + '" title="' + middleColumn.forum.preview.username + '">' + middleColumn.forum.preview.username + '</a></span><br><br>';
				var fourthRow = '<span style="font-size: 10px;">Beitr&auml;ge: 1337</span><br><br>';
				var fifthRow = '<a href="index.php?cont=profile&amp;id=' + middleColumn.forum.preview.userid + '" title="' + middleColumn.forum.preview.username + '"><img src="' + $('.floatl.vcenter.elf.dgray.vcenter:first').html().match(/src="(.+?)" alt/)[1] + '"></a>';
			...
			triggerPreview : function(){
				if(middleColumn.forum.preview.previewIsEnabled == true){
					...
					$('#c_comment').keyup(middleColumn.forum.preview.convertToPreview);
					$('#c_comment').focus(middleColumn.forum.preview.convertToPreview);
					middleColumn.forum.preview.previewIsEnabled = true;
				}
			}
		},
...
rightColumn = {
	...
	*************************/	
	ticker : {
		...
		hideTicker : function(){
			$('#tickr').css('display','none');
			$('div.line3:eq(0)').css('display','none');
			$('div.line3:eq(1)').css('display','none');
	...
	*************************/	
	headlines : {
		...
		hideHeadlines : function(){
			...
			$('div.line3:eq(2)').css('display','none');
	...
	*************************/	
	forum : {
		...
		hideForum : function(){
			$('div.headline_bg:last').css('display','none');
			$('div.cont_box:last').css('display','none');
...
if (content.forum_thread){
	...
	if (options.options.middleColumn_forum_preview == 'checked'){
		middleColumn.forum.preview.insertPreviewHtml();
		document.getElementById('triggerPreview').addEventListener('click', middleColumn.forum.preview.triggerPreview, false);
	}
	
	// Posten im Hintergrund
	if (options.options.middleColumn_forum_postPerAjax == 'checked'){
		$('input[name=submit_thread]').click(function(event) {
			event.preventDefault();
			middleColumn.forum.postPerAjax();
		});
	}
	
	// Posts nachladen
	if (options.options.middleColumn_forum_reloadPosts_readNewPosts == 'checked'){
		middleColumn.forum.readThreadlink();
		middleColumn.forum.readPage();
		middleColumn.forum.reloadPosts.readPostcount();
...
// HTML für die Optionen injekten und Eventhandler zum öffnen und speichern setzen
$('body').append('<div id="userscriptOptions" style="display:none; position:absolute; height:700px; width:600px; left:50%; margin-left:-300px; top:75px; border: 2px solid #00aeed; border-radius: 0px; border-top-left-radius: 20px; border-bottom-left-radius: 20px; background-color: #fff; overflow-x: hidden; overflow-y: scroll;"> <h1 style="color:#000; font-size: 30px; font-variant:small-caps; padding-left: 20px; padding-top:20px; border:none;">Userscript Optionen</h1> <span style="float:right; padding-right:20px;">Verion 2.0.1</span> <div style="padding-left:20px; padding-right:20px; padding-top:20px;"> <table style="border:none; width:100%;"> <tr style="margin:0px; padding:0px; background-color:#fff"> <td style="width:30px;"></td> <td></td> <td style="width:180px;"></td> <td style="width:25px;"></td> </tr> <tr style="background-color:#ccc;"> <td colspan="4" style="color:#fff; font-size:24px; font-variant:small-caps; margin:0px; padding:5px; background-color:#00aeed; width:100%"> Funktionen </td> </tr> <tr style="margin:0px; padding:0px; background-color:#fff"> <td align="left"><img onclick="$(\'.sub_middleColumn_forum_reloadPosts_readNewPosts\').toggle();" src="http://thextor.de/readmore-userscript/img/plus_alt_16x16.png" alt="more" title="Weitere Optionen" /></td> <td align="left">Neue Forenbeitr&auml;ge im Hintergrund nachladen</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="middleColumn_forum_reloadPosts_readNewPosts" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Neue Beitr&auml;ge im Readmore.de-Froum werden automatisch nachgeladen. Ein Refresh des Threads entf&auml;llt somit." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_middleColumn_forum_reloadPosts_readNewPosts"> <td align="left"></td> <td align="left">Zeit zwischen zwei Reloads (in Sekunden)</td> <td align="right"><input type="text" style="padding:0px; margin:0px;" class="userscriptOptions" name="middleColumn_forum_reloadPosts_waitUntilReload" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Bitte Ganzzahlen eintragen. Werte ab 5 Sekunden werden empfohlen, bei einem k&uuml;rzeren Zeitraum kann es unter gewissen Umst&auml;nden zu (tempor&auml;ren) IP-Bans kommen." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_middleColumn_forum_reloadPosts_readNewPosts"> <td align="left"></td> <td align="left">Seite endlos erweitern</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="middleColumn_forum_reloadPosts_endlessPage" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Neue Posts werden einfach eingeblendet. Es muss also nicht mehr auf eine neue Seite gewechselt werden." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_middleColumn_forum_reloadPosts_readNewPosts"> <td align="left"><img onclick="$(\'.sub_middleColumn_forum_reloadPosts_markNewPosts\').toggle();" src="http://thextor.de/readmore-userscript/img/plus_alt_16x16.png" alt="more" title="Weitere Optionen" /></td> <td align="left">Neue Eintr&auml;ge farblich markieren</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="middleColumn_forum_reloadPosts_markNewPosts" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Die neu eingetragenen Posts werden markiert." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#ddd" class="sub_middleColumn_forum_reloadPosts_markNewPosts"> <td align="left"></td> <td align="left">Farbe ausw&auml;hlen (HEX-Code)</td> <td align="right"><input type="text" style="padding:0px; margin:0px;" class="userscriptOptions" name="middleColumn_forum_reloadPosts_markPostColor" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" maxlength="7" alt="info" title="Bitte eine HEX Zahl eingeben. Beispiel: #FFEE11" /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#ddd" class="sub_middleColumn_forum_reloadPosts_markNewPosts"> <td align="left"></td> <td align="left">Ungelesene Posts um Titel / Tab anzeigen</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="middleColumn_forum_reloadPosts_showNewPostsTitle" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" maxlength="7" alt="info" title="Die Anzahl der Ungelesenen Posts im Tab anzeigen." /></td> </tr> <tr style="margin:0px; padding:0px; background-color:#fff"> <td align="left"></td> <td align="left">Vorschaufunktion aktivieren</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="middleColumn_forum_preview" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Vorschau f&uuml;r neue Beitr&auml;ge einbinden." /></td> </tr> <tr style="margin:0px; padding:0px; background-color:#fff"> <td align="left"></td> <td align="left">Beitr&auml;ge ohne Reload senden</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="middleColumn_forum_postPerAjax" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Beitr&auml;ge werden im Hintergrund gepostet. Ein manueller Refresh der Seite entfällt." /></td> </tr> <tr> <td colspan=4>&nbsp;</td> </tr>    <tr style="background-color:#ccc;"> <td colspan="4" style="color:#fff; font-size:24px; font-variant:small-caps; margin:0px; padding:5px; background-color:#00aeed; width:100%"> Optische Ver&auml;nderungen </td> </tr> <tr style="margin:0px; padding:0px; background-color:#fff"> <td align="left"></td> <td align="left">Ticker ausblenden</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="rightColumn_ticker_hideTicker" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Der Ticker wird komplett ausgeblendet." /></td> </tr> <tr style="margin:0px; padding:0px; background-color:#fff"> <td align="left"><img onclick="$(\'.sub_rightColumn_headlines_hideHeadlines\').toggle();" src="http://thextor.de/readmore-userscript/img/plus_alt_16x16.png" alt="more" title="Weitere Optionen" /></td> <td align="left">Schlagzeilen ausblenden</td> <td align="right"></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Bestimmte oder alle Schlagzeilen ausblenden" /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_rightColumn_headlines_hideHeadlines"> <td align="left"></td> <td align="left">Alle ausblenden</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="rightColumn_headlines_hideHeadlines" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet alle Schlagzeilen aus." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_rightColumn_headlines_hideHeadlines"> <td align="left"></td> <td align="left">Counter-Strike ausblenden</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="rightColumn_headlines_hideCounterstrike" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet die Counter-Strike Schlagzeilen aus." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_rightColumn_headlines_hideHeadlines"> <td align="left"></td> <td align="left">Starcraft ausblenden</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="rightColumn_headlines_hideStarcraft" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet die Starcraft Schlagzeilen aus." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_rightColumn_headlines_hideHeadlines"> <td align="left"></td> <td align="left">DotA ausblenden</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="rightColumn_headlines_hideDefenseOfTheAncients" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet die DotA Schlagzeilen aus." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_rightColumn_headlines_hideHeadlines"> <td align="left"></td> <td align="left">League of Legends ausblenden</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="rightColumn_headlines_hideLeagueOfLegends" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet die League of Legends Schlagzeilen aus." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_rightColumn_headlines_hideHeadlines"> <td align="left"></td> <td align="left">Warcraft 3 ausblenden</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="rightColumn_headlines_hideWarcraft3" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet die Warcraft 3 Schlagzeilen aus." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_rightColumn_headlines_hideHeadlines"> <td align="left"></td> <td align="left">Sonstiges ausblenden</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="rightColumn_headlines_hideSonstiges" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet die Sonstigen Schlagzeilen aus." /></td> </tr> <tr style="margin:0px; padding:0px; background-color:#fff"> <td align="left"><img onclick="$(\'.sub_rightColumn_forum_hideForum\').toggle();" src="http://thextor.de/readmore-userscript/img/plus_alt_16x16.png" alt="more" title="Weitere Optionen" /></td> <td align="left">Forum ausblenden und umsortieren</td> <td align="right"></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Das Forum ausblenden oder umsortieren." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_rightColumn_forum_hideForum"> <td align="left"></td> <td align="left">Komplett ausblenden</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="rightColumn_forum_hideForum" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Das komplette Forum ausblenden." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_rightColumn_forum_hideForum"> <td align="left"><img onclick="$(\'.sub_rightColumn_forum_sections\').toggle();" src="http://thextor.de/readmore-userscript/img/plus_alt_16x16.png" alt="more" title="Weitere Optionen" /></td> <td align="left">Foren umsortieren</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="rightColumn_forum_sections" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Ermöglicht das Umsortieren des Forums." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#ddd" class="sub_rightColumn_forum_sections"> <td align="left"></td> <td align="left">An erster Stelle</td> <td align="right"> <select size="1" class="userscriptOptions" name="rightColumn_forum_hideForum_0" style="padding:0px; margin:0px;"> <option value="">Nichts anzeigen</option> <option value="featuredthreads">Featured Threads</option> <option value="esportforen">eSport Foren</option> <option value="technik">Technik</option> <option value="offtopicforen">Offtopic Foren</option> <option value="spiele">Spiele</option> <option value="diablo">Diablo 3</option> </select> </td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Erste Stelle bei der Anzeige der Foren." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#ddd" class="sub_rightColumn_forum_sections"> <td align="left"></td> <td align="left">An zweiter Stelle</td> <td align="right"> <select size="1" class="userscriptOptions" name="rightColumn_forum_hideForum_1" style="padding:0px; margin:0px;"> <option value="">Nichts anzeigen</option> <option value="featuredthreads">Featured Threads</option> <option value="esportforen">eSport Foren</option> <option value="technik">Technik</option> <option value="offtopicforen">Offtopic Foren</option> <option value="spiele">Spiele</option> <option value="diablo">Diablo 3</option> </select> </td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Zweite Stelle bei der Anzeige der Foren." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#ddd" class="sub_rightColumn_forum_sections"> <td align="left"></td> <td align="left">An dritter Stelle</td> <td align="right"> <select size="1" class="userscriptOptions" name="rightColumn_forum_hideForum_2" style="padding:0px; margin:0px;"> <option value="">Nichts anzeigen</option> <option value="featuredthreads">Featured Threads</option> <option value="esportforen">eSport Foren</option> <option value="technik">Technik</option> <option value="offtopicforen">Offtopic Foren</option> <option value="spiele">Spiele</option> <option value="diablo">Diablo 3</option> </select> </td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Dritte Stelle bei der Anzeige der Foren." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#ddd" class="sub_rightColumn_forum_sections"> <td align="left"></td> <td align="left">An vierter Stelle</td> <td align="right"> <select size="1" class="userscriptOptions" name="rightColumn_forum_hideForum_3" style="padding:0px; margin:0px;"> <option value="">Nichts anzeigen</option> <option value="featuredthreads">Featured Threads</option> <option value="esportforen">eSport Foren</option> <option value="technik">Technik</option> <option value="offtopicforen">Offtopic Foren</option> <option value="spiele">Spiele</option> <option value="diablo">Diablo 3</option> </select> </td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Viere Stelle bei der Anzeige der Foren." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#ddd" class="sub_rightColumn_forum_sections"> <td align="left"></td> <td align="left">An f&uuml;nfter Stelle</td> <td align="right"> <select size="1" class="userscriptOptions" name="rightColumn_forum_hideForum_4" style="padding:0px; margin:0px;"> <option value="">Nichts anzeigen</option> <option value="featuredthreads">Featured Threads</option> <option value="esportforen">eSport Foren</option> <option value="technik">Technik</option> <option value="offtopicforen">Offtopic Foren</option> <option value="spiele">Spiele</option> <option value="diablo">Diablo 3</option> </select> </td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="F&uuml;nfte Stelle bei der Anzeige der Foren." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#ddd" class="sub_rightColumn_forum_sections"> <td align="left"></td> <td align="left">An sechster Stelle</td> <td align="right"> <select size="1" class="userscriptOptions" name="rightColumn_forum_hideForum_5" style="padding:0px; margin:0px;"> <option value="">Nichts anzeigen</option> <option value="featuredthreads">Featured Threads</option> <option value="esportforen">eSport Foren</option> <option value="technik">Technik</option> <option value="offtopicforen">Offtopic Foren</option> <option value="spiele">Spiele</option> <option value="diablo">Diablo 3</option> </select> </td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Sechste Stelle bei der Anzeige der Foren." /></td> </tr> <tr style="margin:0px; padding:0px; background-color:#fff"> <td align="left"></td> <td align="left">Wer? Wohin? Warum? ausblenden</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="leftColumn_www_hideWww" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet die komplette Wer? Wohin? Warum? Sektion aus." /></td> </tr> <tr style="margin:0px; padding:0px; background-color:#fff"> <td align="left"><img onclick="$(\'.sub_leftColumn_streams_hideStreams\').toggle();" src="http://thextor.de/readmore-userscript/img/plus_alt_16x16.png" alt="more" title="Weitere Optionen" /></td> <td align="left">Streams ausblenden</td> <td align="right"></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Bestimmte oder alle Streams ausblenden" /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_leftColumn_streams_hideStreams"> <td align="left"></td> <td align="left">Alle Streams ausblenden</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="leftColumn_streams_hideStreams" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet alle Userstreams und Caster aus." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_leftColumn_streams_hideStreams"> <td align="left"></td> <td align="left">Battlefield 3</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="leftColumn_streams_hideSelectedStreams_bf3" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet Battlefield 3 aus." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_leftColumn_streams_hideStreams"> <td align="left"></td> <td align="left">Counterstrike 1.6</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="leftColumn_streams_hideSelectedStreams_cs" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet Counterstrike 1.6 aus." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_leftColumn_streams_hideStreams"> <td align="left"></td> <td align="left">Counterstrike Global Offensive</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="leftColumn_streams_hideSelectedStreams_csgo" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet Counterstrike Global Offensive aus." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_leftColumn_streams_hideStreams"> <td align="left"></td> <td align="left">Counterstrike Source</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="leftColumn_streams_hideSelectedStreams_css" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet Counterstrike Source aus." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_leftColumn_streams_hideStreams"> <td align="left"></td> <td align="left">Defense of the Ancients</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="leftColumn_streams_hideSelectedStreams_dota" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet Defense of the Ancients aus." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_leftColumn_streams_hideStreams"> <td align="left"></td> <td align="left">Defense of the Ancients 2</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="leftColumn_streams_hideSelectedStreams_dota2" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet Defense of the Ancients 2 aus." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_leftColumn_streams_hideStreams"> <td align="left"></td> <td align="left">Diablo 3</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="leftColumn_streams_hideSelectedStreams_d3" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet Diablo 3 aus." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_leftColumn_streams_hideStreams"> <td align="left"></td> <td align="left">Fußball</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="leftColumn_streams_hideSelectedStreams_soccer" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet Fußball aus." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_leftColumn_streams_hideStreams"> <td align="left"></td> <td align="left">Heroes of Newerth</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="leftColumn_streams_hideSelectedStreams_hon" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet Heroes of Newerth aus." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_leftColumn_streams_hideStreams"> <td align="left"></td> <td align="left">Leage of Legends</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="leftColumn_streams_hideSelectedStreams_lol" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet Leage of Legends aus." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_leftColumn_streams_hideStreams"> <td align="left"></td> <td align="left">Quake</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="leftColumn_streams_hideSelectedStreams_ql" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet Quake aus." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_leftColumn_streams_hideStreams"> <td align="left"></td> <td align="left">Starcraft</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="leftColumn_streams_hideSelectedStreams_sc" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet Starcraft aus." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_leftColumn_streams_hideStreams"> <td align="left"></td> <td align="left">Starcraft 2</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="leftColumn_streams_hideSelectedStreams_sc2" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet Starcraft 2 aus." /></td> </tr> <tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_leftColumn_streams_hideStreams"> <td align="left"></td> <td align="left">Warcraft 3</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="leftColumn_streams_hideSelectedStreams_wc3" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet Quake aus." /></td> </tr><tr style="display:none; margin:0px; padding:0px; background-color:#eee" class="sub_leftColumn_streams_hideStreams"> <td align="left"></td> <td align="left">Sonstiges</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="leftColumn_streams_hideSelectedStreams_else" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Blendet den Rest aus." /></td> </tr> <tr> <td colspan=4>&nbsp;</td> </tr>    <tr style="background-color:#ccc;"> <td colspan="4" style="color:#fff; font-size:24px; font-variant:small-caps; margin:0px; padding:5px; background-color:#00aeed; width:100%"> Sonstiges </td> </tr> <tr style="margin:0px; padding:0px; background-color:#fff"> <td align="left"></td> <td align="left">Button zum Runterscrollen anzeigen</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="miscellaneous_buttonScrollDown" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Im Forum wird ein Icon eingefügt, beim betätigen wird zum letzten Post gesprungen." /></td> </tr> <tr style="margin:0px; padding:0px; background-color:#fff"> <td align="left"></td> <td align="left">Button zum Hochscrollen anzeigen</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="miscellaneous_buttonScrollUp" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Im Forum wird ein Icon eingefügt, beim betätigen wird zum ersten Post gesprungen." /></td> </tr> <tr style="margin:0px; padding:0px; background-color:#fff"> <td align="left"></td> <td align="left">Titel/Tab umsortieren</td> <td align="right"><input type="checkbox" style="padding:0px; margin:0px;" class="userscriptOptions" name="miscellaneous_reSortTitle" /></td> <td align="right"><img src="http://thextor.de/readmore-userscript/img/magnifying_glass_16x16.png" alt="info" title="Wenn diese Option aktiviert ist, wird der Threadname an den Anfang des Titels gestellt. Offene Tabs können so besser den verschiedenen Threads zugeordnet werden." /></td> </tr>    <tr> <td align="right" colspan=4> <br/> <input style="padding: 10px;" type="button" value="Abbrechen / Schlie&szlig;en" onclick="$(\'#userscriptOptions\').toggle();" /> &nbsp;&nbsp; <input style="padding: 10px;" type="button" value="Speichern" id="saveUserscriptOptions" /> </td> </tr> </table> </div> </div>');
